...
public class Main {
    ...
    public static void main(String[] args) {
        Scanner in = initializeScanner("src/configuration.txt"); // Initializes scanner object on given file
        if (in == null) return; // If file could not be located, end program

        HashSet<Character> alphabet = getAlphabet(in); // Get all characters of the alphabet from file and save to an alphabet set
        State[] states = initializeStates(in); // Get number of states, activation states, and edges for each states from file
        ...
        while (in.hasNextLine()) {
            ...
            for (char c : word.toCharArray()) {
                if (alphabet.contains(c))
                    stateNumber = states[stateNumber].move(c);
                else {
                    accepted = false;
                    break;
                }
            ...
            else if (states[stateNumber].isActivationState())
    ...
    public static HashSet<Character> getAlphabet(Scanner in) {
        HashSet<Character> alphabet = new HashSet<>(); // Set that contains all the character of the alphabet
        String alphabetLine = in.nextLine(); // Scans in line that contains characters of the alphabet
        for (int i = 0; i < alphabetLine.length(); i += 2) // Adds every other character to avoid commas
            alphabet.add(alphabetLine.charAt(i)); // Adds all characters of the alphabet to an alphabet set
    ...
    public static State[] initializeStates(Scanner in) {
        ...
        State[] states = new State[numberOfStates]; // Create an array that contains the states
        ...
        while (in.hasNextInt()) {
            try {
                ...
                states[startState].addPath(edge, endState);
